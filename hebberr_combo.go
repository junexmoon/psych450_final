// June Moon PSYC450 Final Project
package main
...
func (ev PatsType) String() string                { return "Cancer" }
	...
	{Name: "Base", Desc: "these are the best params", Sheets: params.Sheets{
		"Network": &params.Sheet{
			...
			{Sel: "Layer", Desc: "needs some special inhibition and learning params",
				Params: params.Params{
					"Layer.Learn.AvgL.Gain":    ".5",
					"Layer.Inhib.Layer.Gi":     ".6",
...
type Sim struct {
	...
	Cancer       *etable.Table     `view:"no-inline" desc:"training epoch-level log data"`
...
func (ss *Sim) ConfigEnv() {
	...
	ss.TrainEnv.Table = etable.NewIdxView(ss.Cancer)
	ss.TrainEnv.Validate()
	ss.TrainEnv.Run.Max = ss.MaxRuns // note: we are not setting epoch max -- do that manually

	ss.TestEnv.Nm = "TestEnv"
	ss.TestEnv.Dsc = "testing params and state"
	ss.TestEnv.Table = etable.NewIdxView(ss.Cancer)
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	net.InitName(net, "Diagnosis")
	inp := net.AddLayer2D("Input", 1, 30, emer.Input)
	hid := net.AddLayer4D("Hidden", 2, 4, 5, 6, emer.Hidden)
...
func (ss *Sim) OpenPats() {
	// ss.OpenPatAsset(ss.Cancer, "breastcancer.csv", "Cancer", "Cancer Training patterns")
	ss.OpenPatAsset(ss.Cancer, "breastcancer.tsv", "Cancer", "Cancer Training Patterns")
	err := ss.Cancer.OpenCSV("breastcancer.tsv", etable.Tab)
	if err != nil {
...
func (ss *Sim) ConfigGui() *gi.Window {
	...
	gi.SetAppName("cancer_diagnosis")
	gi.SetAppAbout(`network that can predict tumor as being benign or malignant based upon 10 different cell nucelus attributes`)

	win := gi.NewMainWindow("cancer_diagnosis", "Cancer Diagnosis", width, height)
	ss.Win = win
